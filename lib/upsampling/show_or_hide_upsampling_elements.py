...
def show_or_hide_upsampling_elements(project_name, sample_id, upsampling_running):
  ...
  available_level_names = UI.upsampling.UPSAMPLING_LEVEL_NAMES[:len(levels)]
  ...
  return {
    ...
    UI.upsampling.upsampling_status: gr.update(
      visible = upsampling_running,
    ),
    UI.upsampling.upsampling_level: gr.update(
